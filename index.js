...
const corsOptions = {
  ...
  optionsSuccessStatus: 200, // Some browsers may require this
};
  ...
  ws.on("message", (data) => {
    ...
    if (on["makeOffer"]) {
      videoSocket.clients.forEach((client) => {
        if (client != ws)
          client.send(
            JSON.stringify({ offerMade: { offer: on["makeOffer"].offer } })
          );
      });
      ...
      videoSocket.clients.forEach((client) => {
        if (client != ws)
          client.send(
            JSON.stringify({ answerMade: { answer: on["makeAnswer"].answer } })
          );
      });
...
app.post("/", (req, res) => {
  ...
  // Create a ShareDB document instance
  ...
  doc.fetch((err) => {
    ...
    if (doc.type === null) {
      ...
      // If the document exists, just respond with a success message
